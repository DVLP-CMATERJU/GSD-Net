...
class Spark_Up_Block(nn.Module):        # 2C x H/2 x W/2  --> C x H x W
    def __init__(self,C):
        super(Spark_Up_Block, self).__init__()
        self.up_layer = nn.Sequential(nn.UpsamplingBilinear2d(scale_factor=2),
                                      Spark_Module(2*C,int(C/2),int(C/2),int(C/2)),
                                      Spark_Module(C,int(C/2),int(C/2),int(C/2)))
...
class SegFast(nn.Module):
    def __init__(self,C, noc):
        ...
        self.up_block_3 = Spark_Up_Block(4*C)
        self.up_block_2 = Spark_Up_Block(2*C)
        self.up_block_1 = Spark_Up_Block(C)
        self.classifier = nn.Sequential(nn.UpsamplingBilinear2d(scale_factor=2),
                                        Spark_Module(C,int(C/2),int(C/2),int(C/2)),
                                        nn.Conv2d(C,noc,1))
    def forward(self,x):
