...
def get_model(model_name,noc):
    if model_name.startswith('SegFast'):
        model = models.segfast.SegFast(int(model_name.split('_')[1]),noc)
    if model_name == 'SegNet':
        model = models.segnet.SegNet(3,noc)
    if model_name == 'UNet':
        model = models.unet.UNet(noc)
    return (model)
...
def get_args():
    ...
    parser.add_argument('--model', default = 'SegFast_16',
                        help='SegFast_N|UNet|SegNet')
    parser.add_argument('--dataset_path', default = './data',
                        help='path to dataset')
    parser.add_argument('--noc', type=int, default = 10,
...
def calc_conf(a,b,noc,c_m): # a = actual, b = predicted
    ...
    a = a.data.cpu()
    b = b.data.cpu()
    c_m = c_m + confusion_matrix(a,b,labels = [i for i in range(noc)])
...
class Tester():
    def __init__(self,test_model,test_loader,save_path,noc):
